...
export default function Layout(props) {
  ...
  const updateUserData = async (identifier, password) => {
    ...
    let response = await fetch("https://erp-server-production.up.railway.app/auth/local", {
      method: "POST",
      headers: {
        ...
        if (whoami.user.confirmed) {
          ...
          if (whoami.user.role.type == "public") {
            ...
            Swal.fire({
              icon: "warning",
              title:
                "Vous devez payer vos factures pour bénifier les services.",
              width: 600,
              padding: "3em",
              color: "#171354",
              background: "#fff url(/images/trees.png)",
      ...
      {!user ? (
        ...
        <div
          className={`flex h-screen overflow-hidden relative ${
            i18n.language == "fr" ? "" : "drtl"
          }`}
        >
          {/* Sidebar */}
          <div
            className={
              i18n.language == "fr" ? "" : "absolute right-0 drtl z-40"
            }
          >
            <Sidebar
            ...
            <main>
              <div
                className={`px-4 sm:px-6 lg:px-8 py-8  max-w-7xl mx-auto ${
                  i18n.language == "ar" ? "w-4/5 ml-10" : ""
                }`}
              >
                <div
                  className={`${
                    sessiontype == "Legal" ? "" : ""
                  } p-2  text-white rounded text-right relative mb-10`}
                >
                  {/* <button
                  ...
                  <h3 className="text-gray-900 font-bold">{sessiontype}</h3>
                  <div className="flex rounded-xl bg-gray-300 w-min gap-x-1 p-1 absolute right-1">
                    <div
                      className={`rounded-full ${
                        sessiontype === "Legal" ? "bg-green-500" : ""
                      }  p-1 w-5 h-5 cursor-pointer`}
                      onClick={() => {
                        changeSession();
                      }}
                    ></div>
                    <div
                      className={`rounded-full ${
                        sessiontype === "Illegal" ? "bg-blue-500" : ""
                      }  p-1 w-5 h-5 cursor-pointer`}
                      onClick={() => {
                        changeSession();
                      }}
